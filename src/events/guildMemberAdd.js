...
const IsVerify = global.IsVerify = new Map();
const Discord = require('discord.js');
const renk = require('../renk.json');
const language = require('../dil');
...
module.exports = class GuildMemberAdd extends Event {
    ...
    async run(member, client) {
        const emoji = client.Emoji.json;
        try {
            //----------------Tanımlama-----------------
            if (!member.guild) return;
            let data = await global.dbget(member.guild.id),
            kalanHak = 0, hak = 3, captchaCode,
            sonMesaj, embedDescription, guild = member.guild;
            if (!data || !data['system'] || data['system'] !== 'working') return;
            ...
            member.user.send(new Discord.MessageEmbed().attachFiles(new Discord.MessageAttachment(buffer, 'captchaplus.png')).setDescription(embedDescription).setImage(`attachment://captchaplus.png`).setColor(renk.capt)).then(async (msg) => {
                ...
                const filter = async (m) => {
                    ...
                    if (m.author.id === member.id && m.content.toLowerCase() === captchaCode.toLowerCase()) return true;
                        ...
                        if (kalanHak >= hak) {
                            ...
                            await msg.channel.send(data.language == 'tr' ? `${emoji.cross} Maalesef girişim haklarınız doldu. Yeni bir güvenlik kodu almak için <#${data.verify_channel}> kanalında \`${data.prefix}verify\` komudunu çalıştırın.`: `${emoji.cross} Unfortunately, your venture rights have expired. Run the \`${data.prefix}verify\` command on the <#${data.verify_channel}> channel to get a new security code.`);
                            log('error_hak_bitti');
                            return false;
                        };
                ...
                await msg.channel.awaitMessages(filter, { 
                    ...
                    await msg.channel.send(new Discord.MessageEmbed().setColor(renk.capt)
                    ...
                    if (!member.guild.member(member)) return;
                    await msg.channel.send(data.language == 'tr' ? `${emoji.cross} **Kayıt işleminiz tamamlanamadı.** Yeni bir güvenlik kodu almak için <#${data.verify_channel}> kanalında \`${data.prefix}verify\` komudunu çalıştırın.`: `${emoji.cross} **Your registration could not be completed.** Run the \`${data.prefix}verify\` command on the <#${data.verify_channel}> channel to get a new security code.`);
                    IsVerify.delete(`gived_${member.id}`);
                    log('error_time');
                });
            }).catch(e => {
            ...
            async function log(description) {
                if (data.language == 'tr' && data.log) {
                    ...
                    .setThumbnail(client.user.avatarURL()).setColor(renk.capt).setTimestamp());
                };
            ...
            client.emit('error', error, member.guild);
        };
    }
}